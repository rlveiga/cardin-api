...
def test_create_card(test_client, init_db, token):
    response = test_client.post('/card', json=dict(card_type='black', card_text='Something clever'), headers={'access-token': token})
    ...
    assert data['card']['card_text'] == 'Something clever'
