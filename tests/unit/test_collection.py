...
def test_acquire_collection(test_client, init_db, token):
    collection = Collection.query.filter_by(
        name='collection_1', created_by=2).first()

    response = test_client.post(
        f"/owned_collections/{collection.id}", headers={'access-token': token})
...
def test_acquire_owned_collection(test_client, init_db, token):
    collection = Collection.query.filter_by(
        name='collection_1', created_by=2).first()
...
def test_acquire_collection_unexisting(test_client, init_db, token):
    response = test_client.post(
        f"/owned_collections/42", headers={'access-token': token})
...
def test_disown_collection(test_client, init_db, token):
    collection = Collection.query.filter_by(
        name='collection_1', created_by=2).first()
...
def test_disown_unexisting_collection(test_client, init_db, token):
    response = test_client.delete(
        f"/owned_collections/{42}", headers={'access-token': token})
...
def test_disown_unacquired_collection(test_client, init_db, token):
    collection = Collection.query.filter_by(
        name='collection_1', created_by=2).first()
